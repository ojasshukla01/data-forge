...
- **API**: `GET /api/runs/{run_id}/lineage` returns run_id, run_type, scenario_id, scenario (name, version), pack, custom_schema_id, custom_schema_version, custom_schema_name (when stored for provenance durability), schema_source_type (pack | custom_schema), artifact_run_id, output_dir.
...
- **Contents**: run_id, scenario_id, scenario_version, config (seed, pack or custom_schema_id/custom_schema_version/custom_schema_name, scale, mode, etc.), schema_source_type (pack | custom_schema), config_schema_version, total_rows_generated, duration_seconds, git_commit_sha, platform, python_version, timestamps. When a custom schema is used, custom_schema_name is stored for provenance durability (helps when schema is later deleted). See `data_forge.models.run_manifest` for the full schema.
